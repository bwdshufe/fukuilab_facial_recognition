...
for k = 1:length(orders)
    for is_male = 0:1
        ...
        for i = 1:numel(unique_No_current)
            ...
            cur_rt_mean = data.RT_mean(cur_idx);
            ...
            % 连接同一个参与者的点
            if length(cur_rt_mean) >= 2
                % 按照当前条件顺序排列坐标
                [cur_x_sorted, sort_idx] = sort(cur_x);
                cur_rt_mean_sorted = cur_rt_mean(sort_idx);
                
                % 绘制连线
                line(cur_x_sorted, cur_rt_mean_sorted, 'Color', 'k', 'Linewidth', 1);
            end
            ...
            for j = 1:length(cur_x)
                if strcmp(cur_condition{j}, 'u1')
                    % u1条件用橙色圆圈
                    plot(cur_x(j), cur_rt_mean(j), 'o', 'Color', [0.93, 0.63, 0.13], ...
                        'MarkerSize', 11, 'Linewidth', 1, 'MarkerFaceColor', 'none');
                elseif strcmp(cur_condition{j}, 'f')
                    % f条件用紫色圆圈
                    plot(cur_x(j), cur_rt_mean(j), 'o', 'Color', [0.49, 0.18, 0.56], ...
                        'MarkerSize', 11, 'Linewidth', 1, 'MarkerFaceColor', 'none');
                elseif strcmp(cur_condition{j}, 'u2')
                    % u2条件用蓝色圆圈
                    plot(cur_x(j), cur_rt_mean(j), 'o', 'Color', [0.0, 0.45, 0.74], ...
                        'MarkerSize', 11, 'Linewidth', 1, 'MarkerFaceColor', 'none');
                end
            end
        end
...
% 设置图表属性
ylabel('RT_mean', 'FontSize', 40, 'FontWeight', 'bold');
...
% 添加组标签
text(2, 200, 'Female', 'FontSize', 20, 'FontWeight', 'bold', 'HorizontalAlignment', 'center');
text(6, 200, 'Male', 'FontSize', 20, 'FontWeight', 'bold', 'HorizontalAlignment', 'center');
text(4, 150, 'U→F', 'FontSize', 20, 'FontWeight', 'bold', 'HorizontalAlignment', 'center');
text(10, 200, 'Female', 'FontSize', 20, 'FontWeight', 'bold', 'HorizontalAlignment', 'center');
text(14, 200, 'Male', 'FontSize', 20, 'FontWeight', 'bold', 'HorizontalAlignment', 'center');
text(12, 150, 'F→U', 'FontSize', 20, 'FontWeight', 'bold', 'HorizontalAlignment', 'center');
...
% 保存图像
saveas(gcf, 'rt_mean_visualization_three_cond_modified.png');