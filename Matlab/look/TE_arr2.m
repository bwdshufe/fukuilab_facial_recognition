...
output_dir = 'StatisticalResults/'; % output directory
% ==========================================================
...
for file_idx = 1:length(csv_files)
    ...
    try
        ...
        required_columns = {'No', 'percent', 'condition', 'RT', 'fixation_count', ...
                           'eye_tag1_count', 'nose_tag2_count', 'mouth_tag3_count', 'tag1_only_no_tag2_count', 'validation_accuracy', 'recording_name'};
...
for i = 1:height(unique_combinations_1)
    ...
    if height(subset_data) > 0
        ...
        % Add to result table (added recording_count column)
        new_row = table(current_id, {current_condition}, mean_RT, tag1_ratio, tag2_ratio, tag3_ratio, tag1_only_ratio, mean_fixation_count, num_recordings, ...
                       'VariableNames', {'ID', 'condition', 'mean_RT', 'tag1_ratio', 'tag2_ratio', 'tag3_ratio', 'tag1_only_ratio', 'mean_fixation_count', 'recording_count'});
...
for i = 1:height(unique_combinations_2)
    ...
    if height(subset_data) > 0
        ...
        % Add to result table (added recording_count column)
        new_row = table(current_id, {current_condition}, current_percent, mean_RT, tag1_ratio, tag2_ratio, tag3_ratio, tag1_only_ratio, mean_fixation_count, num_recordings, ...
                       'VariableNames', {'ID', 'condition', 'percent', 'mean_RT', 'tag1_ratio', 'tag2_ratio', 'tag3_ratio', 'tag1_only_ratio', 'mean_fixation_count', 'recording_count'});
