...
whos data.key % data.key の情報（クラス、サイズなど）を表示
% もし必要なら、ここで data.key を cell配列に変換する:
% if isstring(data.key)
%     data.key = cellstr(data.key);
% end
...
for k = 1:length(orders)
    for is_male = 0:1
        ...
        for i = 1:length(current_conditions)
            ...
            current_data = data.Threshold(idx);
        ...
        for i = 1:numel(unique_No_current)
            ...
            cur_threshold = data.Threshold(cur_idx);
            ...
            % 连接同一个参与者的点
            if length(cur_threshold) >= 2
                % 按照当前条件顺序排列坐标
                [cur_x_sorted, sort_idx] = sort(cur_x);
                cur_threshold_sorted = cur_threshold(sort_idx);
                
                % 绘制连线
                line(cur_x_sorted, cur_threshold_sorted, 'Color', 'k', 'Linewidth', 1);
            end
            ...
            for j = 1:length(cur_x)
                if strcmp(cur_condition{j}, 'u1')
                    % u1条件用橙色圆圈
                    plot(cur_x(j), cur_threshold(j), 'o', 'Color', [0.93, 0.63, 0.13], ...
                        'MarkerSize', 11, 'Linewidth', 1, 'MarkerFaceColor', 'none');
                elseif strcmp(cur_condition{j}, 'f')
                    % f条件用紫色圆圈
                    plot(cur_x(j), cur_threshold(j), 'o', 'Color', [0.49, 0.18, 0.56], ...
                        'MarkerSize', 11, 'Linewidth', 1, 'MarkerFaceColor', 'none');
                elseif strcmp(cur_condition{j}, 'u2')
                    % u2条件用蓝色圆圈
                    plot(cur_x(j), cur_threshold(j), 'o', 'Color', [0.0, 0.45, 0.74], ...
                        'MarkerSize', 11, 'Linewidth', 1, 'MarkerFaceColor', 'none');
                end
            end
        end
...
% 添加组标签
text(2, 20, 'Female', 'FontSize', 20, 'FontWeight', 'bold', 'HorizontalAlignment', 'center');
text(6, 20, 'Male', 'FontSize', 20, 'FontWeight', 'bold', 'HorizontalAlignment', 'center');
text(4, 15, 'U→F', 'FontSize', 20, 'FontWeight', 'bold', 'HorizontalAlignment', 'center');
text(10, 20, 'Female', 'FontSize', 20, 'FontWeight', 'bold', 'HorizontalAlignment', 'center');
text(14, 20, 'Male', 'FontSize', 20, 'FontWeight', 'bold', 'HorizontalAlignment', 'center');
...
% 保存图像
saveas(gcf, 'threshold_visualization_three_cond_modified2.png');
